...
;(function($){
    $.fn.responsiveGallery = function(option){
        ...
        $rgItems.click(function(e){
            e.preventDefault();
        });

		function getTransform3dCSS(ts,tx,ty,ry,zIndex,opacity,visibility){
			return {
				'-webkit-transform'	: 'scale('+ts+') translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)',
				'-moz-transform'	: 'scale('+ts+') translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)',
				'-o-transform'		: 'scale('+ts+') translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)',
				'-ms-transform'		: 'scale('+ts+') translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)',
				'transform'			: 'scale('+ts+') translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)',
				'z-index'			: zIndex,
				'opacity'			: opacity,
        ...
        function getTransform7CSS(){
			...
			if(support3d){
				cssArray = [
					getTransform3dCSS('-100%',	-400,	40,		-1,	1,	'visible'),
					...
					getTransform3dCSS('700%', 	-400, 	-40,	-1,	1,	'visible')
				];
			} else if(support2d){
				cssArray = [
					getTransform2dCSS('-100%',	0.6,	'100%',		'50%',	1,	'visible'),
					...
					getTransform2dCSS('700%', 	0.6,	'0%',		'50%',	1,	'visible')
				];
			}
        ...
        // tx,ty,ry,zIndex,opacity,visibility
        // translateX('+tx+') translateZ('+ty+'px) rotateY('+ry+'deg)
		...
		function getTransform3CSS(){
			...
			if(support3d){
				cssArray = [
					getTransform3dCSS('.8', '-148%',	-400, 	100,		0,	0,	'hidden'),

					getTransform3dCSS('.8', '-16%', 	-300, 	50,		1,	1,	'visible'),
					getTransform3dCSS('1',  '116%', 	0, 		0,		2,	1,	'visible'), // 기준
					getTransform3dCSS('.8', '306%', 	-300, 	-50,	1,	1,	'visible'),

					getTransform3dCSS('.8', '438%', 	-400, 	-100,	0,	0,	'hidden')
				];
			} else if(support2d){
				cssArray = [
					getTransform2dCSS('1',  '-148%',	0.65,	'100%',		'50%',	0,	'hidden'),

					getTransform2dCSS('1',  '-16%', 	0.8,	'50%',		'50%',	1,	'visible'),
					getTransform2dCSS('1.1','116%', 	1,		'50%',		'50%',	1,	'visible'),
					getTransform2dCSS('1',  '306%', 	0.8,	'50%',		'50%',	1,	'visible'),

					getTransform2dCSS('1',  '438%', 	0.65,	'0%',		'50%',	0,	'hidden')
				];
			}
		...
		function getTransform1CSS(){
			...
			if(support3d){
				cssArray = [
					getTransform3dCSS('-100%',	-300, 	45,		0,	0,	'visible'),
				...
				cssArray = [
					getTransform2dCSS('-100%',	0.65,	'100%',		'50%',	0,	'visible'),
